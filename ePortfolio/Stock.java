...
/**
 * The class is an object class that acts like a Stock, it is a child class that inherits Investment. 
...
public class Stock extends Investment{
	...
	private final double COMISSION = 9.99;
	 ...
	 */
	public Stock(String newType, String newSymbol, String newName, int newQuant, double newPrice, double newBookVal){
		super(newType, newSymbol, newName, newQuant, newPrice, newBookVal);
	}

	/**
	 *  toString override that formats the Stock with it's correlating data
	 *  @return the formatted string for stock
	 */
	...
	public double calculateBookVal(int units, double price) {
		bookVal += ((double) units * price) + COMISSION;
	...
	public boolean sell(int newQuant, double newPrice) {
		...
		if (remainQuant != 0) { // if it's a partial sale
			...
			saleObj = "Investment: " + symbol + "\nPayment: $" + pay + " Gain: $" + gain;
			empty = false;
		} else { // full sale
			remainBookVal = bookVal * ((double)remainQuant/(double)quantity); // calculating remaining book value
			soldBookVal = bookVal - remainBookVal; // calculating how much is sold
	...
	public double payment(int newQuant, double newPrice) {
		double payVal = ((double) newQuant * newPrice) - COMISSION;
